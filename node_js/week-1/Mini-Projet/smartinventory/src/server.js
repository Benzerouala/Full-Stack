...
/**
 * process.on('SIGINT') - Event listener l signal CTRL+C
 * SIGINT - Signal li ki بعث ملي user يدوس CTRL+C
 * Ki allow shutdown نظيف (clean) dial server
 */
process.on('SIGINT', () => {
  // Ki print message
  console.log('\n🛑 Arrêt du serveur...');
  
  // server.close() - Ki وقف server من قبول requests jdod
  // Callback - Ki execute ba3d server يوقف completely
  server.close(() => {
